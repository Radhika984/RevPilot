...
export type BadgeTone = "neutral" | "risk" | "positive" | "info" | "destructive";
...
export function badgeClassName(tone: BadgeTone): string {
  return `inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${toneClassNames[tone]}`;
}
